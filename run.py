...
import logging, traceback, time, signal, sys, threading
...
if __name__ == '__main__':
    ...
    try:
        ...
        config.connection_pool_maxsize = 5000
        # config.load_kube_config('/etc/kubernetes/admin.conf')
        v1 = client.CoreV1Api()
        v1b1e = client.ExtensionsV1beta1Api()
        q = Queue()
        ...
        worker = applychanges.threadApplyChanges("worker", q)
        worker.daemon = True
        worker.start()
